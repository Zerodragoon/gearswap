...
windower.raw_register_event('incoming chunk', function(id,original,modified,injected,blocked)
    ...
    if id == 0x028 then
        ...
        if packet.Actor == self.id and category == 1 and state.AutoWS.value == 'true' then
			...
			if state.AutoWSList.value == 'Rudra' and player.tp >= 1500 and wsnum == 0 then
                    send_command('input /ws Rudra\'s Storm <t>')
                    wsnum = 0
			end

			if state.AutoWSList.value == 'Mordant' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws Mordant Rime <t>')
                    wsnum = 0
			end

			if state.AutoWSList.value == 'Savage' and player.tp >= 1400 and wsnum == 0 then
                    send_command('input /ws Savage Blade <t>')
                    wsnum = 0
			end
			
			if state.AutoWSList.value == 'Omen' and player.tp >= 1000 and wsnum == 0 then
